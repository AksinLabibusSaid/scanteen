...
(function () {
  ...
  document.getElementById('formStaffAdd')?.addEventListener('submit', async (e) => {
    ...
    if (!data.ok) { msg.classList.add('text-red-600'); msg.textContent = data.error || 'Gagal'; msg.classList.remove('hidden'); return; }
    location.reload();
  });

  document.querySelectorAll('.btn-staff-toggle').forEach(btn => btn.addEventListener('click', async () => {
    const id = parseInt(btn.getAttribute('data-id'), 10);
    const is_active = parseInt(btn.getAttribute('data-next'), 10);
    const res = await fetch(api, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'same-origin', body: JSON.stringify({ action: 'toggle', id, is_active }) });
    const data = await res.json();
    if (!data.ok) { alert(data.error || 'Gagal'); return; }
    location.reload();
  }));

  document.querySelectorAll('.btn-staff-pw').forEach(btn => btn.addEventListener('click', async () => {
    const id = parseInt(btn.getAttribute('data-id'), 10);
    const password = prompt('Sandi baru (min 6 karakter)');
    if (!password || password.length < 6) return;
    const res = await fetch(api, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'same-origin', body: JSON.stringify({ action: 'password', id, password }) });
    const data = await res.json();
    alert(data.ok ? 'Sandi diperbarui' : (data.error || 'Gagal'));
  }));
})();
</script>
