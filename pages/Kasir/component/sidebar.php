...
$q = $db->prepare("SELECT COUNT(*) FROM orders WHERE venue_id = ? AND status = 'pending_payment'");
if ($q) {
    $q->bind_param('i', $venueId);
    $q->execute();
    $q->bind_result($pendingPaymentCount);
    $q->fetch();
    $q->close();
}
?>
            ...
            <a href="?page=orders"
               class="<?= $activePage === 'orders' ? 'active-nav' : 'text-gray-500 hover:bg-gray-50' ?> flex items-center justify-between px-4 py-3 rounded-r-lg transition-colors group">
                ...
                <?php if ($pendingPaymentCount > 0): ?>
                    <div class="flex items-center justify-center min-w-[22px] h-[22px] bg-[var(--brand)] text-white px-1.5 rounded-full shadow-sm flex-shrink-0" style="color: #ffffff !important; font-size: 11px !important; font-weight: 700 !important; line-height: 1 !important;" title="<?= $pendingPaymentCount ?> pesanan menunggu pembayaran">
                        <?= $pendingPaymentCount ?>
                    </div>
                <?php endif; ?>
            </a>
